...
<template>
  ...
  >
    ...
    <h-desc labelWidth="110px" size="small" :column="1">
      ...
      <h-desc-item label="模板名称">
        {{ detail.name ? detail.name : '--' }}
      </h-desc-item>
      <h-desc-item label="模版分组">
        {{ detail.groupCode == 'data' ? '数据模板' : detail.groupCode == 'form' ? '表单模板' : '报告模板' }}
      </h-desc-item>
      <h-desc-item label="模版分类" v-if="detail.groupCode == 'report'">
        {{ detail.reportType == 'cover' ? '封面' : detail.reportType == 'report' ? '报告' : '--' }}
      </h-desc-item>
