  ...
  >
    <div v-if="mode === 'sidemenu'" class="header" :class="theme">
      <div class="header-tool">
        ...
        <span v-if="device === 'desktop'">{{ loginWelcome }}</span>
        <span v-else>HIFAR</span>
      </div>
...
import JChat from '@/components/JChat'
...
export default {
  ...
  methods: {
    getTitleText() {
      getAction('/SysSwitchBusiness/queryByItemKeyPrefix', {itemKey: 'titleText'}).then((result) => {
        if (result.code === 200) {
          result.data.map((item) => {
            if (item.itemKey === 'titleText') {
              this.titleText = item.itemValue
            }
          })
        }
      })
