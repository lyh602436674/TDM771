/*
 * @Author: 赵峰
 * @Date: 2021-09-15 14:59:22
 * @LastEditTime: 2021-11-05 17:12:53
 * @LastEditors: 赵峰
 * @Descripttion: webOffice工具类
 * @FilePath: \hifar-platform-client\src\plugins\webOffice.js
 */
function LoadCtrl(obj) {
  ...
  window.WebOffice = document.getElementById('WebOffice')
  window.WebOffice.UserName = '北京华天海峰科技股份有限公司'
  window.WebOffice.Authorizer = 'www.officectrl.com'
...
function ShowViewPage(param, fileUrl, token) {
  let pre = 'WebOffice://|Officectrl|'// 智能窗打开的固定参数
  let origin = decodeURI(window.location.origin)
  let path = '/user/webOfficeView'
  let strUrl
  let v = getBrowser()
  if (v === 1) { // 当浏览器返回值为1时定义为使用智能窗方式打开网址
    strUrl = pre + origin + path + '?url=' + fileUrl + '&param=' + JSON.stringify(param) + '&token=' + token
    window.open(encodeURI(strUrl), '_self')
  } else { // 当浏览器返回值1以外的数据时采用传统方式打开网址
    strUrl = origin + path + '?url=' + fileUrl + '&param=' + JSON.stringify(param) + '&token=' + token
    window.open(encodeURI(strUrl), '_blank')
  }
}

function ShowEditPage(fileUrl,token, baseUrl,fileUrlAuth,system) {
  let pre = 'WebOffice://|Officectrl|'// 智能窗打开的固定参数
  let origin = decodeURI(window.location.origin)
  let strUrl
  let v = getBrowser()
  if (v === 1) { // 当浏览器返回值为1时定义为使用智能窗方式打开网址
    strUrl = pre + origin + '/edit.html' + '?url=' + fileUrl + '&baseUrl=' + baseUrl  + '&token=' + token + '&fileUrlAuth=' + fileUrlAuth + '&system=' + system 
    window.open(encodeURI(strUrl), '_self')
  } else { // 当浏览器返回值1以外的数据时采用传统方式打开网址
    strUrl = pre + origin + '/edit.html' + '?url=' + fileUrl + '&baseUrl=' + baseUrl  + '&token=' + token + '&fileUrlAuth=' + fileUrlAuth + '&system=' + system
    window.open(encodeURI(strUrl), '_blank')
...
export {
  LoadCtrl, ShowViewPage, ShowEditPage, SetMenu, isNotCopy, WebSignature
}